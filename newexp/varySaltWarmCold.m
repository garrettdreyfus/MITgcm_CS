function runShelfDepthCheck()
  ...
  experiment_parameters.initial_state = 'warm';
  ...
  for i = 1:1
    experiment_parameters.initial_state = 'warm';
    experiment_parameters.saltflux_factor = factors(i);
    path_part1 = convertStringsToChars(strcat("experiments/varysf",int2str(rng_seed),"/"));
    path_part2 = convertStringsToChars(strcat("imroved-warmstart-warm-sf",int2str(factors(i)*100)));
    ...
    experiment_parameters.saltflux_factor = factors(i);
    path_part1 = convertStringsToChars(strcat("experiments/varysf",int2str(rng_seed),"/"));
    path_part2 = convertStringsToChars(strcat("imroved-coldstart-warm-sf",int2str(factors(i)*100)));
    full_path = convertStringsToChars(strcat("../",path_part1,path_part2));
    newexp(path_part1,path_part2,experiment_parameters);
    cd(full_path);
    system('sh upload_to_cluster.sh')

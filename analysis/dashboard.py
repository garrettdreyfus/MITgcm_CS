...
def FStheory(fname,xval):
    ...
    for k in data.keys():
        if k != "ts":
            try:
                data[k] = np.nanmean(data[k][data["ts"]>0])#2.5])
            except:
                data[k]=np.nan
    if "gprime" not in data.keys():
        data["gprime"] = np.nan
...
def steadyStateAverageSimple(fname,xval,fig,ax1,title="",color="blue",marker="o"):
    ...
    for k in data.keys():
        if k != "ts":
            try:
                data[k] = np.nanmean(data[k][data["ts"]>0])
            except:
                data[k]=np.nan
    if "gprime" not in data.keys():
        data["gprime"] = np.nan
...
def circulationFigure(fname,description,times=np.array([])):
    ...
    uvel = np.nanmean(ds.UVEL.values[ts>0],axis=0)
    vvel = np.nanmean(ds.VVEL.values[ts>0],axis=0)

    theta= np.nanmean(ds.THETA.values[ts>0],axis=0)
    salt= np.nanmean(ds.SALT.values[ts>0],axis=0)
    ...
    for i in range(theta.shape[1]):
        for j in range(theta.shape[2]):
            zc = np.where(np.diff(np.sign(salt[:,i,j]-34.5)))[0]
            if len(zc)>0:
...
def meltmapmovie(fname,description,times=np.array([])):
    ...
    with moviewriter.saving(fig, fpath+"-meltmap.mp4" , dpi=100):
        for k in tqdm(range(int(ds.UVEL.values.shape[0]))):
            ...
            melt[~mask]=np.nan
            thetaavg[~mask]=np.nan
            frame = ax1.pcolormesh(-melt,cmap="jet",vmin=-0.0001,vmax=0.0001)
            if k==0:
                cb = plt.colorbar(frame)
            moviewriter.grab_frame()
            #cb.remove()
...
def crossSectionAnim(fname,description,quant="THETA",res=1,dim="zonal"):
    ...
    #ds = open_mdsdataset(fname,prefix="T",ignore_unknown_vars=True,extra_variables = extra_variables)
    #print(ds)
    ...
    moviewriter = FFMpegFileWriter(fps=1)
    if quant!="DENS":
        ds = open_mdsdataset(fname,prefix=quant,ignore_unknown_vars=True,extra_variables = extra_variables)
        times = ds.time.values
        #mask = np.float64(times)>=243900000000000
        if dim == "zonal":
            #zonal_average = ds.where(ds.hFacC==1).mean(dim="XC",skipna=True)
            zonal_average = ds.isel(XC=61)
            ys = zonal_average.YC.values
        if dim == "meridional":
            zonal_average = ds.isel(YC=135)
            ys = zonal_average.XC.values
        ...
        length = zvals.shape[0]
    if quant=="DENS":
        ds = open_mdsdataset(fname,prefix=["SALT_inst","THETA_inst"],ignore_unknown_vars=True,extra_variables = extra_variables,iters=times)
        #zonal_average = ds.where(ds.hFacC==1).isel(XC=100)
        if dim == "zonal":
            zonal_average = ds.where(ds.hFacC==1).mean(dim="XC",skipna=True)
            ys = zonal_average.YC.values
        if dim == "meridional":
            zonal_average = ds.isel(YC=150)
            ys = zonal_average.XC.values
        shortname, fpath = outPath(fname) 
        fig.suptitle(shortname)
        zs = zonal_average.Z.values
        #tmin, tmax = np.nanmin(zonal_average[quant]), np.nanmax(zonal_average[quant])
        zvals = (zonal_average["SALT_inst"].values,zonal_average["THETA_inst"].values)
        length = zvals[0].shape[0]
    with moviewriter.saving(fig, fpath+quant+"|"+dim+".mp4" , dpi=250):
        print(fpath+quant+"|"+dim+".mp4")
        for k in tqdm(range(0,length,res)):
            if quant == "DENS":
                frame = ax1.pcolormesh(ys,zs,gsw.sigma0(zvals[0][k,:,:],zvals[1][k,:,:]),cmap="jet",vmin=27.5,vmax=27.9)
            elif quant in ["THETA","THETA_inst","T"]:
                #frame = ax1.pcolormesh(ys,zs,zvals[k,:,:],cmap="seismic",vmin=-2.401,vmax=-1.799)
                frame = zvals[k,:,:]
                frame[frame==0] = np.nan
                frame = ax1.pcolormesh(ys,zs,frame,cmap="jet")
            elif quant in ["SALT","SALT_inst","S"]:
                frame = zvals[k,:,:]
                frame[frame==0] = np.nan
                frame = ax1.pcolormesh(ys,zs,frame,cmap="jet")
            else:
                #frame = ax1.pcolormesh(ys,zs,zvals[k,:,:],cmap="jet")
                frame = zvals[k,:,:]
                #frame[frame==0] = np.nan
                frame = ax1.pcolormesh(ys,zs,frame,cmap="jet")
            cb = plt.colorbar(frame)
...
def bottomAnim(fname,description,times=np.array([]),quant="THETA_inst",res=5):
    fig,ax1 = plt.subplots()
    extra_variables = dict( SHIfwFlx = dict(dims=["k","j","i"], attrs=dict(standard_name="Shelf Fresh Water Flux", units="kg/m^3")))
    times=getIterNums(fname)
    ds = open_mdsdataset(fname,ignore_unknown_vars=True,extra_variables = extra_variables,iters=times)
    ...
    with moviewriter.saving(fig, fpath+"-"+quant+"-bot.mp4", dpi=250):
        for k in tqdm([0]+list(range(quantvals.shape[0]))+[-1]):
            ...
            frame = ax1.pcolormesh(ds.XC.values,ds.YC.values,znew,cmap="jet",vmin=-1.9,vmax=-1.7)
            #frame = ax1.pcolormesh(ds.XC.values,ds.YC.values,znew,cmap="jet",vmin=34.6,vmax=34.8)
            #frame = ax1.pcolormesh(ds.XC.values,ds.YC.values,znew,cmap="jet",vmin=-0.1,vmax=0.1)
...
#meltmapmovie("/home/garrett/Projects/MITgcm_CS/experiments/sushelfdepth-16/d700/results/","")
#meltMapAverage("/home/garrett/Projects/MITgcm_CS/experiments/sushelfdepth-16/d700/results","")
#crossSectionAnim("/home/garrett/Projects/MITgcm_CS/experiments/sushelfdepth-16/d700/results/","")
#bottomAnim("/home/garrett/Projects/MITgcm_CS/experiments/sushelfdepth-16/d700/results/","")
#crossSectionAnim("/home/garrett/Projects/MITgcm_CS/experiments/builttofail-16/d700/results/","",quant="T",dim="meridional")
#fname = "/home/garrett/Projects/MITgcm_CS/experiments/builttofail-16/d700/results/"
#ds = open_mdsdataset(fname,prefix=["T","S"],ignore_unknown_vars=True)
#times = ds.time.values
#mask = np.float64(times)>=243900000000000
###for i in range(np.shape(ds.S)[0]):
    ##print(times[i])
    ##print(np.sum(np.isnan(ds.T.values[i,:])))
#for i in range(1,np.shape(ds.S)[1]):
    #if np.isnan(ds.T.values[-1][-i]).any() or True:
        #print(np.sum(np.isnan(ds.T.values[-1][-i])))
        #plt.pcolormesh(ds.XC,ds.YC,ds.T.values[-1][-i])
        #plt.show()
    
crossSectionAnim("/home/garrett/Projects/MITgcm_CS/experiments/builttofail-16/ewobcs700/results/","")
...
#plt.show()
#plt.show()
#crossSectionAverage("/home/garrett/Projects/MITgcm_CS/experiments/reference/at125/results","Reference")
#plt.show()
#circulationFigure("/home/garrett/Projects/MITgcm_CS/experiments/reference/at125/results","Reference")
#plt.show()
